...
int main() {
	...
	while(1) {
		x1 = i2cInt16(hndl, bufAddr) / 16384.f; // 16384를 사용하는 이유 : 2^14 로 나누는 것을 의미
		y1 = i2cInt16(hndl, bufAddr+2) / 16384.f; // 16384를 사용하는 이유 : 2^14 로 나누는 것을 의미
		z1 = i2cInt16(hndl, bufAddr+4) / 16384.f; // 16384를 사용하는 이유 : 2^14 로 나누는 것을 의미
		
		x2 = i2cInt16(hndl, bufAddr+8) / 131.f; // 16384를 사용하는 이유 : 2^14 로 나누는 것을 의미
		y2 = i2cInt16(hndl, bufAddr+10) / 131.f; // 16384를 사용하는 이유 : 2^14 로 나누는 것을 의미
		z2 = i2cInt16(hndl, bufAddr+12) / 131.f; // 16384를 사용하는 이유 : 2^14 로 나누는 것을 의미
